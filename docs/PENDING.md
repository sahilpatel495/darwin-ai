...
- **Narration claims.** A live answer named the wrong department as "highest" while every number was genuine, under a High-confidence badge. The number-grounding check verifies numbers, not the claim built from them, and the eval grades the table, not the sentence. Fix: compute highest/lowest/first/last rows deterministically, check superlatives and label-number pairings in the sentence, fall back to a rank-aware template. Full brief: the stopped `verity-correctness` workflow script (narration-claims task). *Until this lands, treat ranking sentences with care and trust the chart and table.*
